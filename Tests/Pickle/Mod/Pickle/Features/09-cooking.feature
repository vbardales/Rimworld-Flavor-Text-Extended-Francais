...
# scenario checks what must hold for any name. The film (`@film`) and the two captures are the
...
# `-Language French -IncludeWip -Filter '09-cooking.feature'`. Whether a colonist of the fixture can cook
# is checked first, so a failure names its cause instead of timing out.
@wip @review @film @slow @timeout:180
