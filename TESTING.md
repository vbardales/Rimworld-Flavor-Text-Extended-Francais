...
| --- | --- | --- | --- | --- | --- |
| 1 | `sans-facultatifs`, English | The minimal set plus PickleTools ScreenshotMode (`-DepMap wsl-deps.sans-facultatifs.map`) | English | 01, 02, 04, 05, 06, 19 | **non-regression replay 2026-09-25 after the 1.0.1 publication (request `e893`): 21/21, `exitReason: passed`.** Before that: replayed 2026-09-23 with ScreenshotMode, suite `846c156`: `exitReason: passed`, 21 passed, 0 failed, 29 skipped (French and `@wip` scenarios of other passes); clean captures opened (`docs/runs/2026-09-23.md`).** Feature 19 (exact dishes) now plays here too. |
| 2 | `sans-facultatifs`, French | The same set plus ScreenshotMode | French | 03, then 01, 04, 05, 06, 19, 07: seven launches under one lock | **non-regression replay 2026-09-25 after the 1.0.1 publication (request `b7be`): seven launches, 27/27, all `exitReason: passed`.** Before that: replayed 2026-09-23 with ScreenshotMode, suite `846c156`: seven launches, all `exitReason: passed`, 27 scenarios, 0 failed (`docs/runs/2026-09-23.md`); raw reports of launches 1 to 6 were purged before copying, the counts and captures are kept.** |
| 2b | `sans-facultatifs`, French, cooking | The same set plus `PickleTools/FilmTicks` (`-DepMap wsl-deps.cuisson-film.map`) | French | 09, filmed | **green (2026-09-25, request `20260925-153507-017-6072`, 1/1, `exitReason: passed`, one attempt, 200 s, tree `5b99cfa`).** Four earlier attempts, none a verdict on the mod: the stock 120 s bill wait (2026-09-24), a survival ration taken for the dish, a pawn list read off the main thread, and (2026-09-25) the scenario watchdog at 120 s (`@watch` slowed the game to about 13 ticks a second), then the game killed by another session's test script. The fifth ran with a 600 s scenario watchdog (`docs/runs/2026-09-25.md`). The film and the colony-view capture are for the owner to judge. |
...
- **For 1.0.1:** the two lavish meals of feature 07 (side dish in lower case after a French joint): **green on 2026-09-25**
  (`docs/runs/2026-09-25.md`, no negative control). The cooking pass 2b (feature 09) is green (request 6072, 2026-09-25).
...
- Review is limited to captured images or the cooking film: features 04, 06, 07, 08, 12 and 14 use
  ...
  ingredients (F14) and the restart are played (passes 8, 9, 5 and 6). Cooking with a colonist is green (2b, 2026-09-25).
