...
| 2b | `sans-facultatifs`, French, cooking | The same set plus `PickleTools/FilmTicks` (`-DepMap wsl-deps.cuisson-film.map`) | French | 09, filmed | **not green yet.** Four attempts, none a verdict on the mod: the stock 120 s bill wait (2026-09-24), a survival ration taken for the dish, a pawn list read off the main thread, and (2026-09-25) the scenario watchdog at 120 s (`@watch` slowed the game to about 13 ticks a second), then the game killed by another session's test script. Request `20260925-153507-017-6072` (the 1.0.1 candidate tree) queued with a 600 s scenario watchdog (`docs/runs/2026-09-25.md`). |
